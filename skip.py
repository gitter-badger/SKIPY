...
def background_number(s):
    return get_costume_number(s.project.stage)
...
def replace_item_with(s, index, list_, item):
    if index == 'last':
        ...
        index = random.randint(1, len(list_.items))
...
class PygameScreen(Screen):
    ...
    def set_project(self, project):
        ...
        Screen.set_project(self, project)
        if project.name:
    ...
    def draw_sprite(self, sprite, onto_surface, offset=None):
        ...
        if isinstance(sprite, kurt.Stage):
            ...
            pos = self.pos_to_screen(bounds(sprite).topleft)
            #if sprite.direction != 90 and sprite.size != 100:
            angle = -(sprite.direction - 90)
            scale = sprite.size / 100.0
            surface = pygame.transform.rotozoom(surface, angle, scale)
    ...
    def touching_mouse(self, sprite):
        ...
        (x, y) = self.pos_to_screen(bounds(sprite).topleft)
    ...
    def touching_sprite(self, sprite, other):
        ...
        (x, y) = self.pos_to_screen(bounds(sprite).topleft)
        (ox, oy) = self.pos_to_screen(bounds(other).topleft)
